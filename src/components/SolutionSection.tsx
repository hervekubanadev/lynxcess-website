...
const SolutionSection = () => (
  <section id="solution" className="relative py-32 overflow-hidden">
    <div className="absolute top-0 left-1/3 w-96 h-96 rounded-full bg-accent/3 blur-[120px]" />
        ...
        {features.map((f, i) => (
          <ScrollReveal key={i} delay={i * 0.1}>
            <div className="glass-panel-neon p-8 text-center hover-lift h-full">
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-xl glass-panel mb-6">
                <f.icon className="text-primary" size={24} />
              </div>
              <h3 className="font-display font-semibold text-foreground mb-3">{f.title}</h3>
