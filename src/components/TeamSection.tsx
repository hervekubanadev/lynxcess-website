...
const TeamSection = () => {
  ...
  return (
    <section id="team" className="relative py-32 overflow-hidden">
      <div className="absolute bottom-0 right-0 w-80 h-80 rounded-full bg-secondary/5 blur-[120px]" />
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {members.map((m, i) => (
            <ScrollReveal key={i} delay={i * 0.1}>
              ...
              >
                <div className="w-16 h-16 rounded-full gradient-neon flex items-center justify-center text-primary-foreground font-display font-bold text-lg mb-4 group-hover:neon-glow-cyan transition-shadow">
                  {initials(m.name)}
                </div>
                <h3 className="font-display font-semibold text-foreground">{m.name}</h3>
                <p className="text-sm text-primary font-body mt-1">{m.role}</p>
              </button>
            </ScrollReveal>
          ))}
        </div>
      ...
      <AnimatePresence>
        {selected && (
          ...
          >
            ...
            <motion.div
              className="relative glass-panel-neon neon-glow-cyan p-8 md:p-10 max-w-lg w-full rounded-2xl"
            ...
            >
              ...
              <div className="w-20 h-20 rounded-full gradient-neon flex items-center justify-center text-primary-foreground font-display font-bold text-2xl mb-6">
                {initials(selected.name)}
              </div>

              <h3 className="text-2xl font-display font-bold text-foreground">{selected.name}</h3>
              <p className="text-primary font-display font-medium mt-1">{selected.role}</p>
